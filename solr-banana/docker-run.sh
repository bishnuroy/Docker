...
function start() {
  ...
  if [ -n "${ZK_HOST}" ]; then
    ...
    [[ ! -f ${SOLR_HOME}/solr.xml ]] && { tar -C / -xf ${HOME}/solr-backup.tar; chown -R solr:solr ${SOLR_HOME}; }
    ...
    ${SOLR_PREFIX}/bin/solr create -c banana-int -d ${SOLR_SERVER_DIR}/solr-webapp/webapp/banana/resources/banana-int-solr-5.0;
  else
